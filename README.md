...
- Local bindings named `console` (function parameters, imports, `let`/`const`/`var` declarations, and class/function declarations) are always preserved. Bindings introduced only by TypeScript declaration-space constructs (`namespace console`, `enum console`, `import console = ...`) and by a `case` clause are not detected.
