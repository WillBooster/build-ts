...
An opinionated, zero-config build tool for Node.js applications and libraries written in TypeScript, powered by [Rolldown](https://rolldown.rs/).

## Highlights

- **Zero config** — point it at a package and it bundles `src/index.{ts,tsx,cts,mts}` into `dist/` with sensible defaults (minification, sourcemaps, and tree-shaking enabled).
...
| Option           | Alias | Default  | Description                                                                 |
| ---------------- | ----- | -------- | --------------------------------------------------------------------------- |
| `--js-extension` | `-j`  | `either` | Which format uses the `.js` extension: `either`, `both`, or `none`. Other formats use `.cjs` / `.mjs`. |
